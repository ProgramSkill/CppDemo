...
| English | 中文 | 说明 / Notes |
|---------|------|--------------|
| Relay | 继电器 | 电控开关,用于控制高功率负载 |
| Coil voltage | 线圈电压 | 继电器线圈的工作电压(如5V, 12V) |
| Contact rating | 触点额定值 | 触点可承受的最大电流/电压 |
| Snubber circuit | 缓冲电路 | RC网络,保护触点免受电弧损坏 |
| Inductive load | 感性负载 | 电机、螺线管等带线圈的负载 |
| Resistive load | 阻性负载 | 灯泡、加热器等纯电阻负载 |
| Capacitive load | 容性负载 | LED驱动器、电子镇流器等 |
| Back EMF | 反电动势 | 感性负载断电时产生的反向电压 |
| Solid-state relay (SSR) | 固态继电器 | 无机械触点的电子开关 |
| Zero-crossing detection | 过零检测 | 在交流电过零点切换,减少干扰 |
| H-bridge | H桥 | 用于控制电机方向的电路 |
| Solenoid | 电磁阀/螺线管 | 电磁驱动的机械装置 |
| Inrush current | 浪涌电流 | 设备启动瞬间的大电流 |
| Arcing | 电弧 | 触点断开时的火花放电 |
| Contact welding | 触点焊接 | 过载导致触点粘连 |
...
| English | 中文 | 说明 / Notes |
|---------|------|--------------|
| Seven-segment display | 七段数码管 | 显示数字0-9的LED显示器 |
| Common cathode | 共阴极 | 所有阴极连接到GND |
| Common anode | 共阳极 | 所有阳极连接到VCC |
| Segment | 段 | 数码管的单个LED段(a-g, dp) |
| Decimal point (dp) | 小数点 | 数码管的第8个段 |
| Multiplexing | 多路复用/扫描 | 快速切换显示多个数字 |
| Refresh rate | 刷新率 | 显示更新频率(Hz) |
| Ghosting | 重影 | 切换时的残影现象 |
| Lookup table | 查找表 | 预先计算的数据数组 |
| Display buffer | 显示缓冲区 | 存储待显示数据的数组 |
| Digit selection | 位选择 | 选择要显示的数字位 |
...
## 🔊 Buzzer and Audio / 蜂鸣器与音频

| English | 中文 | 说明 / Notes |
|---------|------|--------------|
| Buzzer | 蜂鸣器 | 音频信号发生器 |
| Active buzzer | 有源蜂鸣器 | 内置振荡器,固定频率 |
| Passive buzzer | 无源蜂鸣器 | 需要外部信号驱动 |
| Tone | 音调 | 特定频率的声音 |
| Pitch | 音高 | 声音的高低 |
| Musical note | 音符 | 音乐中的音调单位 |
| Octave | 八度 | 音乐中的频率倍数关系 |
| Tempo | 节拍 | 音乐速度(BPM) |
| Whole note | 全音符 | 最长的基本音符时值 |
| Half note | 二分音符 | 全音符的一半 |
| Quarter note | 四分音符 | 全音符的四分之一 |
| Eighth note | 八分音符 | 全音符的八分之一 |
| Resonant frequency | 谐振频率 | 蜂鸣器最响的频率 |
| Sound pressure | 声压 | 声音强度,单位dB |
| Square wave | 方波 | 简单的音频波形 |
| Waveform | 波形 | 信号的形状 |
| Piezo | 压电 | 压电陶瓷蜂鸣器 |
...
## ⌨️ Matrix Keypad / 矩阵键盘

| English | 中文 | 说明 / Notes |
|---------|------|--------------|
| Matrix keypad | 矩阵键盘 | 行列扫描式键盘 |
| Keypad | 键盘 | 多按键输入设备 |
| Row | 行 | 矩阵键盘的横向线 |
| Column | 列 | 矩阵键盘的纵向线 |
| Scanning | 扫描 | 依次检测按键状态 |
| Key mapping | 按键映射 | 行列位置到按键值的对应 |
| Ghost key | 重影键 | 3键同按时的误检测 |
| Diode matrix | 二极管矩阵 | 防重影的键盘设计 |
| Typematic | 连击/自动重复 | 长按自动重复输入 |
| Multi-key detection | 多键检测 | 同时检测多个按键 |
| Key repeat | 按键重复 | 类似键盘的重复功能 |

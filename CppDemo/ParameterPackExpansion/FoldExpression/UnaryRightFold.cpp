...
int main() {
    std::cout << "=== 一元右折叠示例 ===" << std::endl << std::endl;

    // 示例1: 加法
    int result1 = sum_right(1, 2, 3, 4);
    std::cout << "sum_right(1, 2, 3, 4) = " << result1 << std::endl;
    // 展开为: 1 + (2 + (3 + 4)) = 10

    // 示例2: 减法（展示右折叠的结合性）
    int result3 = subtract_right(10, 5, 3, 1);
    std::cout << "subtract_right(10, 5, 3, 1) = " << result3 << std::endl;
    // 展开为: 10 - (5 - (3 - 1)) = 10 - (5 - 2) = 10 - 3 = 7

    // 示例3: 乘法
    int result4 = multiply_right(2, 3, 4);
    std::cout << "multiply_right(2, 3, 4) = " << result4 << std::endl;
    // 展开为: 2 * (3 * 4) = 2 * 12 = 24
    ...
    // 示例6: 字符串连接
    std::string result9 = concat_right(std::string("Hello"), std::string(" "), std::string("World"), std::string("!"));
    std::cout << "concat_right(\"Hello\", \" \", \"World\", \"!\") = " << result9 << std::endl;
    // 展开为: "Hello" + (" " + ("World" + "!"))
